/*class Solution {
    public int maximumRemovals(String s, String p, int[] removable) {
        ...
        int l=0,h=removable.length-1,mid=0,ans=-1;
        while(l<=h)
        {
            mid=(l+h)>>1;
            System.out.println("mid is " + mid);
            if(valid(a1,a2,mid,s,removable))
            {
                ans=mid;
                l=mid+1;
            }
    ...
    boolean valid(char[] a1,char[] a2,int mid,String s,int[] removable)
    {
        for(int i=0;i<=mid;i++)
        {
            a1[removable[i]]='*';
